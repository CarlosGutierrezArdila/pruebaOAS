...
type Actividad struct {
	...
	FCreaccion    time.Time    `orm:"column(f_creaccion);type(date)"`
	FLimite       time.Time    `orm:"column(f_limite);type(date)"`
	Descripcion   string       `orm:"column(descripcion)"`
	Estado        string       `orm:"column(estado)"`
	IdResponsable *Responsable `orm:"column(id_responsable);rel(fk)"`
}
